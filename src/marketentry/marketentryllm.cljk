(ns marketentry.marketentryllm
  ...
  the Malawi (MWI) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "seeking-msme-set-aside?=" (:seeking-msme-set-aside? e)
                        " msme-declared-category=" (:msme-declared-category e)
                        " annual-turnover-mwk=" (:annual-turnover-mwk e)
                        " employee-count=" (:employee-count e)
                        " assets-mwk=" (:assets-mwk e)
                        " tpin-record-verified?=" (:tpin-record-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tpin-record? e))
                              (:tpin-record-verified? e)))
                   0.9 0.3)}))
