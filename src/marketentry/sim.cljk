...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MWI, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up msme-set-aside-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (msme-set-aside-ineligible -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tpin-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tpin-record-unverified -> HARD hold) ==")
