  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Lilongwe Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Lilongwe Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest msme-set-aside-ineligible-is-held-and-unoverridable
  (testing "declared :seeking-msme-set-aside? true but declared turnover/employees/assets satisfy fewer than 2 of the 3 band criteria for its own declared category -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:msme-set-aside-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest tpin-record-unverified-is-held-and-unoverridable
  (testing "unverified MRA TPIN record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tpin-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
